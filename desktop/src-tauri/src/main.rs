// Thin shell: starts the Python backend (`pcassist ui --no-browser --idle 0 --json-ready`), reads the JSON ready line
...
fn backend_exe(app: &AppHandle) -> Option<PathBuf> {
    ...
    if let Ok(dir) = app.path().resource_dir() {
        candidates.push(dir.join("backend").join("pcassist.exe"));
    }
    candidates.push(PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../../build/dist/pcassist/pcassist.exe")));
    candidates.into_iter().find(|p| p.exists())
}

fn start_backend(app: &AppHandle) -> Result<(Child, String), String> {
    let exe = backend_exe(app).ok_or("backend (pcassist.exe) not found")?;
...
fn main() {
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let open = MenuItem::with_id(app, "open", "Open pcassist", true, None::<&str>)?;
            ...
            std::thread::spawn(move || match start_backend(&handle) {
                ...
                Err(e) => {
                    if let Some(w) = handle.get_webview_window("main") {
                        ...
                        let _ = w.eval(&format!("document.body.innerText = \"pcassist could not start: {msg}\""));
                    }
                }
            });
            Ok(())
